(function ($) {
    ...
    const pages = ['about', 'team', 'services', 'portfolio', 'contact'];
    ...
    function init_ED_PageBackground() {
        ...
        if (body.hasClass('kenburns-background')) {
            ...
            const kenburnsDisplayBackdrops = false;
            const kenburnsBackgrounds = [
    ...
    function init_ED_Navigation() {
        ...
        if (!(1024 >= getWindowWidth() || body.hasClass('mobile'))) {
            ...
            btnLoadContent.on('click', function (e) {
                ...
                if (pages.indexOf(target.substr(1)) > -1) {
                    document.getElementById(target.substr(1)).style.zIndex = ++counter;
                    document.getElementById(target.substr(1)).style.right = 0;
                }
            });
    ...
    jQuery(document).ready(function ($) {
        ...
        const d = new Date();
        copyright.innerHTML = "&copy;" + d.getFullYear().toString() + " BlueMatrix Technologies&trade; - All Rights Reserved";
    });
